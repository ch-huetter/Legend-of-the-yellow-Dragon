...
public class TavernApiController {
    ...
    public ResponseEntity<RestResponse> rest () {
        ...
        if (playerCharacter.getHealth().equals(playerCharacter.getMaxHealth())) {
        ...
        return ResponseEntity.ok(new RestResponse(ResponseStatus.SUCCESS, null, playerCharacter));
    }

    public record RestResponse(ResponseStatus status, RestErrors error, PlayerCharacter playerCharacter) {
